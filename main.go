...
func NewGame() *Game {
	return &Game{day: 1, warmth: 70, hunger: 75, wood: 3, food: 2, weather: "clear", x: 320, y: 250, rng: rand.New(rand.NewSource(time.Now().UnixNano())), message: "Explore the island. Press E near something useful.", nodes: []Node{{110, 120, "wood", false}, {170, 260, "wood", false}, {500, 150, "wood", false}, {535, 255, "wood", false}, {95, 305, "wood", false}, {350, 105, "wood", false}, {420, 210, "wood", false}, {450, 285, "food", false}, {260, 245, "camp", false}}}
}
...
func (g *Game) interact() {
	for i := range g.nodes {
		...
		if n.kind == "wood" && g.wood < 12 {
			g.wood++
			n.used = true
			g.message = "You gather dry wood."
			return
		}
		if n.kind == "food" {
			g.food++
			n.used = true
			g.message = "You find something edible among the rocks."
			return
		}
	}
	g.message = "Nothing useful here."
}
...
func (g *Game) canGather() bool {
	...
	for _, n := range g.nodes {
		if !n.used && (n.kind == "wood" || n.kind == "food") && abs(float64(n.x)-g.x) < 42 && abs(float64(n.y)-g.y) < 42 {
			return true
		}
	}
	return false
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	text.Draw(screen, "Q eat", basicfont.Face7x13, 390, 420, g.actionColor(g.food > 0))
	text.Draw(screen, "R restart", basicfont.Face7x13, 445, 420, color.White)
	text.Draw(screen, g.message, basicfont.Face7x13, 32, 442, color.RGBA{240, 220, 160, 255})
}
func (g *Game) Layout(_, _ int) (int, int) { return 640, 455 }
func main() {
	ebiten.SetWindowSize(640, 455)
